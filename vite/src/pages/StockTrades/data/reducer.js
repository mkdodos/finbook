...
export const initialState = {
  ...
  // 1. 新增：搜尋條件狀態
  searchParams: {
    // startDate: dayjs().format("YYYY-MM-DD"), // 排程日 (起)
    startDate: "", // 排程日 (起)
    // endDate: nextWeek.format("YYYY-MM-DD"), // 排程日 (訖)
    machine: "", // 機台編號
    keyword: "", // 關鍵字 (客戶/單號)
  },
};
...
export const reducer = (state, action) => {
  switch (action.type) {
    ...
    case "LOAD_SUCCESS":
      ...
      //   if (!acc[item.stock_id]) acc[item.stock_id] = [];
      //   acc[item.stock_id].push(item);
      //   return acc;
      // }, {});
      const groupedData = action.payload.data.reduce((acc, item) => {
        const key = `${item.stock_id} ${item.name}`; // 組合成 "2330 台積電"
        (acc[key] ??= []).push(item);
        return acc;
      }, {});
      const groupedDataByDate = action.payload.data.reduce((acc, item) => {
